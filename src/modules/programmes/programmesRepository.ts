...
export const findAll = async () => {
    const [rows] = await client.query<RowDataPacket[]>("SELECT * FROM PROGRAMMES");
    return rows;
};
...
export const update = async (
    ...
    const [result] = await client.query<ResultSetHeader>(
        "UPDATE PROGRAMMES SET NOM = ?, OBJECTIF = ?, DUREE = ?, ID_COACH = ? WHERE ID_PROGRAMME = ?",
        [nom, objectif, duree, idCoach, id],
    );
